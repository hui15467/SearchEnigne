...
* AbstractIndex中还定义了从docId到docPath之间到映射关系
 * AbstractIndex的具体实现类
 * 必须实现下面接口:
...
public class Index extends AbstractIndex {
    ...
    public void addDocument(AbstractDocument document) {
        ...
        map.forEach((key,val)->{if(this.termToPostingListMapping.containsKey(key)){
        this.termToPostingListMapping.get(key).add(val);
        }else{
            AbstractPostingList p1=new PostingList();
            p1.add(val);
            this.termToPostingListMapping.put(key,p1);
        }
        });
    }
    /**
